...
import { FIXED_DONATION_AMOUNT_TRIGGER, FixedDonationAmountTrigger } from "./FixedDonationAmountTrigger";
import { LESS_THAN_DONATION_AMOUNT_TRIGGER, LessThanDonationAmountTrigger } from "./LessThanDonationAmountTrigger";
import { RANDE_DONATION_AMOUNT_TRIGGER, RangeDonationAmountTrigger } from "./RangeDonationAmountTrigger";
...
export class TriggersStore {
  ...
  public get available(): TriggerType[] {
    ...
    this._added.forEach((t) => {
      if (t.type.type === FIXED_DONATION_AMOUNT_TRIGGER.type) {
        additionalFilters.push((t: TriggerType) => t.type !== RANDE_DONATION_AMOUNT_TRIGGER.type);
        additionalFilters.push((t: TriggerType) => t.type !== LESS_THAN_DONATION_AMOUNT_TRIGGER.type);
      }
      if (t.type.type === RANDE_DONATION_AMOUNT_TRIGGER.type) {
        additionalFilters.push((t: TriggerType) => t.type !== FIXED_DONATION_AMOUNT_TRIGGER.type);
      }
      if (t.type.type === LESS_THAN_DONATION_AMOUNT_TRIGGER.type) {
        additionalFilters.push((t: TriggerType) => t.type !== FIXED_DONATION_AMOUNT_TRIGGER.type);
      }
    })
    return this._types
      .filter((t) => additionalFilters.every((f) => f(t)))
      .filter((t) => !this._added.find((a) => a.type.type === t.type));
  }
  ...
  public loadTrigger(trigger: Trigger): Trigger {
    switch (trigger.type.type) {
      case "fixed-donation-amount":
        return new FixedDonationAmountTrigger((trigger as FixedDonationAmountTrigger).amount);
      case "at-least-donation-amount":
        return new RangeDonationAmountTrigger((trigger as RangeDonationAmountTrigger).min);
      case "less-than-donation-amount":
        return new LessThanDonationAmountTrigger((trigger as LessThanDonationAmountTrigger).amount);
      case "system":
