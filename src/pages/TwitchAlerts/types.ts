...
import { Element, ElementContainer, ElementData } from "../../components/Element/Element";
...
export class TwitchAlert implements ElementContainer {
  ...
  public moveDown(id: string) {
    ...
    if (allElements.at(firstScope.start)?.advancedLevel !== allElements.at(secondScope.start)?.advancedLevel) {
      return;
    }
  ...
  private swap(firstScope: Scope, secondScope: Scope){
    ...
    log.debug({ firstScope, secondScope, result: this._data.elements
              .sort((a, b) => a.order - b.order)
              .map((element) => { return { name: element.name, order: element.order};}) }, "moving down elements");
  }
